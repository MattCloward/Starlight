...
execute at @a[gamemode=!creative,gamemode=!spectator] unless block ~ ~-1 ~ sculk unless block ~ ~-2 ~ sculk unless block ~ ~ ~ sculk_vein unless block ~ ~-1 ~ sculk_vein run scoreboard players operation @a[sort=nearest,limit=1] onSculkTimer = #blocksattackhandler sentienceTimer
...
# TODO use #blocksattackhandler decayTimer or a different value to make this value customizable
