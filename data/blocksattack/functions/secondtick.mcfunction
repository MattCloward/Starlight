...
execute at @a[gamemode=!creative,gamemode=!spectator] unless block ~ ~-1 ~ sculk unless block ~ ~-2 ~ sculk unless block ~ ~ ~ sculk_vein unless block ~ ~-1 ~ sculk_vein run scoreboard players operation @a[sort=nearest,limit=1] onSculkTimer = #handler sentienceTimer
...
# bloom commands
execute as @e[type=experience_orb] at @s if score @s entityAge >= #handler xpAge run function blocksattack:bloom
...
# invulnerability timer for boss
execute as @e[type=magma_cube,tag=brain,tag=shifter,scores={invulnerable=1..}] run scoreboard players remove @s invulnerable 1