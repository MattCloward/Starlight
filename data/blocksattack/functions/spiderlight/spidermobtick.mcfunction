...
execute as @e[type=spider,tag=spider_light,nbt={NoAI:0b}] at @s if entity @p[limit=1,sort=nearest,distance=16..,gamemode=!spectator] run function blocksattack:freezeblockmob
# unfreeze spider entity if close to player
execute as @e[type=spider,tag=spider_light,nbt={NoAI:1b}] at @s if entity @p[limit=1,sort=nearest,distance=..16,gamemode=!spectator] run data merge entity @s {NoAI:0b,Silent:0b}
